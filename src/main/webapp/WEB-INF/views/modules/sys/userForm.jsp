...
<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
        ...
        <div class="controls">
            ...
            <c:if test="${empty user.loginName}">
                <form:input path="loginName" htmlEscape="false" maxlength="50"
                            placeholder="登录名唯一，注册后不可修改" class="required userName"/>
            </c:if>
            <c:if test="${not empty user.loginName}">
                <form:input path="loginName" htmlEscape="false" maxlength="50"
                            readonly="true" class="required userName"/>
            </c:if>
            <span class="help-inline"><font color="red">*</font> </span>
        </div>
    </div>
    ...
    <c:if test="${not empty user.id}">
        ...
        <%--系统管理员才能修改到期时间,add by HL--%>
        <shiro:hasRole name="sysadmin">
            <div class="control-group">
                <label class="control-label">到期时间</label>
                <div class="controls">
                    <c:if test="${not empty user.expireDate}">
                        <input name="expireDate" type="text" maxlength="20"
                               class="input-medium Wdate required"
                               value="<fmt:formatDate value="${user.expireDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
                               onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
                    </c:if>
                    <%--<c:if test="${empty user.expireDate}">--%>
                        <%--<input name="expireDate" type="text" maxlength="20"--%>
                        <%--class="input-medium Wdate required"--%>
                        <%--value="<fmt:formatDate value="${fns:getMonthLater(1)}" pattern="yyyy-MM-dd HH:mm:ss"/>"--%>
                        <%--onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>--%>
                        <%--</c:if>--%>
                    <span class="help-inline"><font color="red">*</font>用户的试用期默认一个月，到期后不能登录，请谨慎设置</span>
                </div>
            </div>
        </shiro:hasRole>
        <div class="control-group">
